...
export async function checkEligibility(
    ...
    const intra = createIntraClient(accessToken, userId);

    
    const [cursusData, projects, scaleTeams, dbUser, activeRecords] = await Promise.all([
        ...
        prisma.disciplinaryRecord.findMany({ // Registros disciplinares activos do usuário, para avaliar sanções e bloqueios, filtrando apenas aqueles que estão activos (isActive: true) para garantir que apenas sanções em vigor sejam consideradas na avaliação de elegibilidade.
    ...
    const criteria: EligibilityCriterion[] = [
        ...
        // Art. 3a — Ser Cadete da 42 Luanda
        {
            ...
            reason: !dbUser ? 'Utilizador não encontrado no sistema' : // Se o usuário existe mas não é um estudante, retorna um motivo específico para falha, indicando que apenas estudantes têm acesso.
                dbUser.role !== 'STUDENT' ? 'Apenas estudantes têm acesso' : undefined,
        },
        ...
        // Art. 3b — Concluído a 2ª Órbita do Holy Graph
        {
            ...
            reason: !mainCursus ? 'Cursus 42 não encontrado' :
                (mainCursus.level < 3) ? `Nível actual: ${mainCursus.level.toFixed(2)} (mín: 3.00)` : undefined,
        },
        ...
        // Art. 3c — Ritmo ≤ 22
        {
            ...
            label: 'Ritmo ≤ 22',
            article: 'Art. 3.º-c',
            passed: (mainCursus?.level ?? 99) <= 22,
            reason: (mainCursus?.level ?? 0) > 22 ?
                `Nível ${mainCursus?.level.toFixed(2)} excede o limite de 22` : undefined,
        },
        ...
        // Art. 3d — Média de avaliações > 3/mês
        {
            ...
            article: 'Art. 3.º-d',
            passed: (() => {
                const validEvals = scaleTeams.filter(s => s.final_mark !== null)
                ...
                const avg = validEvals.reduce((s, e) => s + (e.final_mark ?? 0), 0) / validEvals.length
                return avg > 3
            })(),
            reason: (() => {
                const validEvals = scaleTeams.filter(s => s.final_mark !== null)
                if (validEvals.length === 0) return 'Sem avaliações nos últimos 30 dias'
                const avg = validEvals.reduce((s, e) => s + (e.final_mark ?? 0), 0) / validEvals.length
                return avg <= 3 ? `Média actual: ${avg.toFixed(1)} (mín: 3.0)` : undefined
            })(),
        },
        ...
        // Art. 3f — Situação académica regular
        {
            ...
            passed: (() => {
                ...
                // garantindo que apenas os projetos que atendem a esses critérios sejam considerados como falhas graves, 
        ...
        // Art. 3g — Comportamento adequado (baseado em reports resolvidos)
        {
            ...
            reason: dbUser?.isBlocked ? 'Acesso bloqueado pelo staff' : undefined,
        },
    ...
    await Promise.all([
        prisma.user.update({
            ...
            data: {
                isEligible,
                eligibilityNote: isEligible ? null :
                    criteria.filter(c => !c.passed).map(c => c.label).join(', '),
                lastSyncAt: new Date(),
            },
        }),
        ...
        redis.setex(cacheKey, CACHE_TTL, JSON.stringify(result)).catch((err: any) => {
            console.error("Erro ao gravar no Upstash:", err);
        }),
    ])
...
export function isWithinOperatingHours(): { allowed: boolean; reason?: string } {
    const now = new Date()
    ...
    if (day === 0 || day === 6) {
        return { allowed: false, reason: 'A PlayStation não está disponível ao fim-de-semana.' }
    }
    if (hour < 8 || hour >= 17) {
        return { allowed: false, reason: 'Fora do horário. Disponível Seg-Sex das 08h00 às 17h00.' }
    }
